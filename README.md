...
| `homma capture` | Copies what you said to an agent session out of its transcript into the workspace's capture store, word for word, with every question you were asked, its options and your answer. It takes only what came after the last capture from that session, so running it twice writes nothing the second time, and it writes the file and leaves committing it to you |
